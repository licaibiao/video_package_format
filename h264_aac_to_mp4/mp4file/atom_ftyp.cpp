	...
	: MP4Atom("ftyp")
{
	...
	AddProperty( /* 1 */new MP4Integer32Property("minorVersion"));

	MP4Integer32Property* pCount = new MP4Integer32Property("compatibleBrandsCount"); 
	pCount->SetImplicit();
	AddProperty(pCount); /* 2 */

	MP4TableProperty* pTable = new MP4TableProperty("compatibleBrands", pCount);
...
void MP4FtypAtom::Generate() 
{
	...
	MP4StringProperty* pBrandProperty = (MP4StringProperty*)((MP4TableProperty*)m_pProperties[3])->GetProperty(0);
	ASSERT(pBrandProperty);
	pBrandProperty->AddValue("mp42");
	//pBrandProperty->AddValue("isom");
	pBrandProperty->AddValue("avc1");
