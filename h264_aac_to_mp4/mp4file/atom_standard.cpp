...
MP4StandardAtom::MP4StandardAtom (const char *type) : MP4Atom(type)
{
	...
	*/
	...
	else if (ATOMID(type) == ATOMID("co64")) 
	{
		...
		pTable->AddProperty(new MP4Integer64Property("chunkOffset"));

	} 
	else if (ATOMID(type) == ATOMID("cpil") || ATOMID(type) == ATOMID("covr")) 
	{ /* Apple iTunes */
	...
	else if (ATOMID(type) == ATOMID("cprt")) 
	{
		...
		AddProperty(new MP4StringProperty("notice"));
	}
	...
	*/
	...
	{ // bytes of immediate data
		AddProperty( new MP4Integer64Property("bytes"));

	}
	else if (ATOMID(type) == ATOMID("disk")) 
	{ /* Apple iTunes */
		ExpectChildAtom("data", Required, OnlyOne);
	} 
	...
	*/
	...
	{
		AddVersionAndFlags();
		AddProperty(new MP4DescriptorProperty(NULL, MP4ESDescrTag, 0, Required, OnlyOne) );
	} 
	else if (ATOMID(type) == ATOMID("frma")) 
	{
		AddProperty( /* 0 */new MP4Integer32Property("data-format"));			
	/*
	* g???
	*/
	} 
	...
	{
		AddVersionAndFlags();
		AddProperty(new MP4DescriptorProperty(NULL, MP4FileIODescrTag, 
					  MP4FileODescrTag, 
					  Required, OnlyOne));
	}
	...
	else if (ATOMID(type) == ATOMID("meta")) 
	{ // iTunes
		AddVersionAndFlags(); /* 0, 1 */
		ExpectChildAtom("hdlr", Required, OnlyOne);
	...
	else if (ATOMID(type) == ATOMID("trpy") || ATOMID(type) == ATOMID("tpyl")) 
	{
