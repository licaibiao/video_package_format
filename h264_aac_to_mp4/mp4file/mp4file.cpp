...
MP4File::MP4File(u_int32_t verbosity)
{
	...
	m_nSampleTimeStamp = MP4_INVALID_TIMESTAMP;
	 ...
	 )
{
	...
	if (add_ftyp != 0) 
		MakeFtypAtom(majorBrand, minorVersion, supportedBrands, supportedBrandsCount);

	CacheProperties();

	// create mdat, and insert it after ftyp, and before moov
	InsertChildAtom(m_pRootAtom, "mdat",  add_ftyp != 0 ? 1 : 0);
...
void MP4File::Open(const char* fmode)
{
	...
	if (m_mode == 'r') 
	{
		...
		m_orgFileSize = ftell(m_pFile);;
		m_fileSize = m_orgFileSize;
		fseek(m_pFile, nPos, SEEK_CUR);
		//CFileStatus fs;
//    		if( !CFile::GetStatus( CString(m_fileName), fs))
//    		{
//    			throw new MP4Error(errno, "stat failed", "MP4Open");
//    		}
//    		m_orgFileSize = m_fileSize = fs.m_size;
	} 
	else  
...
const char* MP4File::TempFileName()
{
	...
	GetTempFileName(".", // dir. for temp. files 
					...
					strName);        // buffer for name 
	//wcstombs(m_tempFileName, strName, _MAX_PATH);

	strcpy(m_tempFileName, strName );
#endif
 ...
 #ifdef _WIN32
 	rc = ::DeleteFile((newFileName));
 	if (rc == 0) {
 		rc = MoveFile((oldFileName), (newFileName));
 	}
 #else
 	//rc = MoveFile((oldFileName), (newFileName));
...
void MP4File::AddTrackToIod(MP4TrackId trackId)
{
	...
	m_pRootAtom->FindProperty("moov.iods.esIds", (MP4Property**)&pDescriptorProperty);
	ASSERT(pDescriptorProperty);

	MP4Descriptor* pDescriptor = pDescriptorProperty->AddDescriptor(MP4ESIDIncDescrTag);
	ASSERT(pDescriptor);

	MP4Integer32Property* pIdProperty = NULL;
	pDescriptor->FindProperty("id", (MP4Property**)&pIdProperty);
	ASSERT(pIdProperty);
 ...
 void MP4File::RemoveTrackFromIod(MP4TrackId trackId, bool shallHaveIods)
{
	...
	for (u_int32_t i = 0; i < pDescriptorProperty->GetCount(); i++) 
	{
	  /* static */char name[32];
		...
		MP4Integer32Property* pIdProperty = NULL;
		pDescriptorProperty->FindProperty(name, (MP4Property**)&pIdProperty);
		ASSERT(pIdProperty);
...
{
  MP4TrackId trackId = AddVideoTrackDefault(timeScale, sampleDuration, width, height, "avc1");

	SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.avc1.width", width);
	SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.avc1.height", height);

	/* shouldn't need this 为什么没有添加avcC */
	// AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.avc1"), "avcC");
	
	// 设置avcC参数
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.avc1.avcC.AVCProfileIndication",AVCProfileIndication);
	
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.avc1.avcC.profile_compatibility", profile_compat);

	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.avc1.avcC.AVCLevelIndication",AVCLevelIndication);

	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.avc1.avcC.lengthSizeMinusOne",sampleLenFieldSizeMinusOne);
...
// sps
bool MP4File::AddH264SequenceParameterSet (MP4TrackId trackId, const uint8_t *pSequence,uint16_t sequenceLen)
{
	MP4Atom *avcCAtom = FindAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.avc1.avcC"));
	MP4BitfieldProperty *pCount;
	MP4Integer16Property *pLength;
	MP4BytesProperty *pUnit;
  
	bool bSpsNum = avcCAtom->FindProperty("avcC.numOfSequenceParameterSets", (MP4Property **)&pCount);
	bool bSpsLen =  avcCAtom->FindProperty("avcC.sequenceEntries.sequenceParameterSetLength",(MP4Property **)&pLength ) ;
	bool bSpsNal = avcCAtom->FindProperty("avcC.sequenceEntries.sequenceParameterSetNALUnit", (MP4Property **)&pUnit);
	if ( ( bSpsNum == false) || (bSpsLen== false) || (bSpsNal  == false) )
	{
...
// PPs
bool MP4File::AddH264PictureParameterSet (MP4TrackId trackId,const uint8_t *pPict, uint16_t pictLen )
{
	MP4Atom *avcCAtom = FindAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.avc1.avcC"));
	...
	if ((avcCAtom->FindProperty("avcC.numOfPictureParameterSets",(MP4Property **)&pCount) == false) 
	||(avcCAtom->FindProperty("avcC.pictureEntries.pictureParameterSetLength",(MP4Property **)&pLength) == false) 
	||(avcCAtom->FindProperty("avcC.pictureEntries.pictureParameterSetNALUnit",(MP4Property **)&pUnit) == false)) 
	{
		VERBOSE_ERROR(m_verbosity, WARNING("Could not find avcC picture table properties"));
		return false;
	}
	uint32_t count = pCount->GetValue();
	...
	if (count > 0) 
	{
		...
		for (uint32_t index = 0; index < count; index++) 
		{
			  if (pLength->GetValue(index) == pictLen) 
			  {
					...
					{
						VERBOSE_WRITE(m_verbosity,fprintf(stderr, "picture matches %d\n", index));
						free(seq);
						return true;
					}
					free(seq);
	...
	pCount->IncrementValue();
	VERBOSE_WRITE(m_verbosity,fprintf(stderr, "new picture added %d\n", pCount->GetValue()));

	return true;
}

void  MP4File::SetH263Vendor(MP4TrackId trackId, u_int32_t vendor)
{
	SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.s263.d263.vendor", vendor);
}

void MP4File::SetH263DecoderVersion( MP4TrackId trackId,u_int8_t decoderVersion)
{
	SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.s263.d263.decoderVersion", decoderVersion);
}
...

{
  MP4TrackId trackId = AddVideoTrackDefault(timeScale, 
					    ...
					    "s263");
	// width
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.s263.width", width);
	
	// height
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.s263.height", height);
	
	//h263Level
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.s263.d263.h263Level", h263Level);
	
	// h263Profile
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.s263.d263.h263Profile", h263Profile);

	// Add the bitr atom
	AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.s263.d263"), "bitr");
	...
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsz.sampleSize", sampleDuration);
...
MP4TrackId MP4File::AddHintTrack(MP4TrackId refTrackId)
{
	...
	MP4TrackId trackId = AddTrack(MP4_HINT_TRACK_TYPE, GetTrackTimeScale(refTrackId));
	...
	SetTrackIntegerProperty(trackId,"mdia.minf.stbl.stsd.rtp .tims.timeScale", GetTrackTimeScale(trackId));
...
MP4TrackId MP4File::FindTrackId(u_int16_t trackIndex, const char* type, u_int8_t subType)
{
	if (type == NULL)  
...
u_int16_t MP4File::FindTrakAtomIndex(MP4TrackId trackId)
{
	...
	throw 
		new MP4Error("Track id %d doesn't exist", "FindTrakAtomIndex",trackId); 
...
MP4SampleId MP4File::GetSampleIdFromTime(MP4TrackId trackId, MP4Timestamp when, bool wantSyncSample)
{
	return m_pTracks[FindTrackIndex(trackId)]->GetSampleIdFromTime(when, wantSyncSample);
}

MP4Timestamp MP4File::GetSampleTime( MP4TrackId trackId, MP4SampleId sampleId)
{
	MP4Timestamp timestamp;
	m_pTracks[FindTrackIndex(trackId)]->GetSampleTimes(sampleId, &timestamp, NULL);
	
	return timestamp;
}

MP4Duration MP4File::GetSampleDuration( MP4TrackId trackId, MP4SampleId sampleId)
{
	MP4Duration duration;
	m_pTracks[FindTrackIndex(trackId)]-> GetSampleTimes(sampleId, NULL, &duration);
	return duration; 
}

MP4Duration MP4File::GetSampleRenderingOffset( MP4TrackId trackId, MP4SampleId sampleId)
{
	return m_pTracks[FindTrackIndex(trackId)]->GetSampleRenderingOffset(sampleId);
}
		...
		)
{
	m_pTracks[FindTrackIndex(trackId)]->ReadSample(
		sampleId, ppBytes, pNumBytes,
		pStartTime, pDuration, pRenderingOffset, pIsSyncSample
		);
}

void MP4File::ReadSampleEx(MP4TrackId trackId, MP4SampleId sampleId,u_int32_t SampleCount,
		u_int8_t** ppBytes, u_int32_t* pNumBytes,u_int32_t* pRealSampleCount)
{
	m_pTracks[FindTrackIndex(trackId)]->ReadSampleEx(sampleId, 
		SampleCount, 
		ppBytes, 
		pNumBytes,
		pRealSampleCount
...
void MP4File::SetSampleTimeStamp(MP4Timestamp nTimeStamp)
{
	m_nSampleTimeStamp = nTimeStamp ;
}
	...
	)
{
	...
	m_pTracks[FindTrackIndex(trackId)]->WriteSample(pBytes, 
		numBytes,
		m_nSampleTimeStamp,
		duration, 
		renderingOffset, 
	...
	m_pModificationProperty->SetValue(m_nSampleTimeStamp );//MP4GetAbsTimestamp()
}

void MP4File::SetSampleRenderingOffset(MP4TrackId trackId, MP4SampleId sampleId, MP4Duration renderingOffset)
{
	ProtectWriteOperation("MP4SetSampleRenderingOffset");

	m_pTracks[FindTrackIndex(trackId)]->SetSampleRenderingOffset(sampleId, renderingOffset);
...
char* MP4File::MakeTrackName(MP4TrackId trackId, const char* name)
{
	...
	if (name == NULL || name[0] == '\0') 
		snprintf(m_trakName, sizeof(m_trakName), "moov.trak[%u]", trakIndex);
	else 
		snprintf(m_trakName, sizeof(m_trakName), "moov.trak[%u].%s", trakIndex, name);
...
void MP4File::SetTrackIntegerProperty(MP4TrackId trackId, const char* name, int64_t value)
{
...
void MP4File::SetTrackFloatProperty(MP4TrackId trackId, const char* name, float value)
{
...
void MP4File::AppendSessionSdp(const char* sdpFragment)
{
	...
	char* newSdpString =(char*)MP4Malloc(strlen(oldSdpString) + strlen(sdpFragment) + 1);
...
{
  MP4Atom *pChild;
  MP4Atom *pAtom = FindAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd"));
 
  if (pAtom->GetNumberOfChildAtoms() != 1) 
  {
    VERBOSE_ERROR(m_verbosity,  fprintf(stderr, "track %d has more than 1 child atoms in stsd\n", trackId));
    return NULL;
  }
...
{
	MP4SampleId numSamples = GetTrackNumberOfSamples(trackId);
	u_int64_t 	msDuration = ConvertFromTrackDuration(trackId, 	GetTrackDuration(trackId), MP4_MSECS_TIME_SCALE);
...
{
	return GetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*[0].channels");
}

// true if media track encrypted according to ismacryp
bool MP4File::IsIsmaCrypMediaTrack(MP4TrackId trackId)
{
    if (GetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*.sinf.frma.data-format") != (u_int64_t)-1) 
		return true;
...
void MP4File::GetTrackESConfiguration(MP4TrackId trackId, u_int8_t** ppConfig, u_int32_t* pConfigSize)
{
...
void MP4File::GetTrackVideoMetadata(MP4TrackId trackId, u_int8_t** ppConfig, u_int32_t* pConfigSize)
{
	GetTrackBytesProperty(trackId, "mdia.minf.stbl.stsd.*[0].*.metadata",ppConfig, pConfigSize);
}

void MP4File::SetTrackESConfiguration(MP4TrackId trackId, const u_int8_t* pConfig, u_int32_t configSize)
{
	...
	if (pInfoProperty == NULL)
	{
		...
		MP4Descriptor* pConfigDescr = pConfigDescrProperty->AddDescriptor(MP4DecSpecificDescrTag);
		
		pConfigDescr->Generate();

		pConfigDescrProperty->FindProperty("decSpecificInfo[0].info",(MP4Property**)&pInfoProperty);
...
void MP4File::GetTrackH264ProfileLevel (MP4TrackId trackId, uint8_t *pProfile, uint8_t *pLevel)
{
	*pProfile = GetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*[0].avcC.AVCProfileIndication");
	*pLevel = GetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*[0].avcC.AVCLevelIndication");
}

void MP4File::GetTrackH264LengthSize (MP4TrackId trackId, uint32_t *pLength)
{
	*pLength = 1 +  GetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*[0].avcC.lengthSizeMinusOne");
}
	...
	)
{
	...
	bool bNumofSps = avcCAtom->FindProperty("avcC.numOfSequenceParameterSets",(MP4Property **)&pSeqCount);
	bool bSpsLen = avcCAtom->FindProperty("avcC.sequenceEntries.sequenceParameterSetLength",(MP4Property **)&pSeqLen);
	bool bSpsNalU = avcCAtom->FindProperty("avcC.sequenceEntries.sequenceParameterSetNALUnit",(MP4Property **)&pSeqVal);
	if ( !bNumofSps  || !bSpsLen  || !bSpsNalU ) 
	{
		VERBOSE_ERROR(m_verbosity, WARNING("Could not find avcC properties"));
		return false;
	}

	uint8_t **ppSeqHeader = (uint8_t **)malloc((pSeqCount->GetValue() + 1) * sizeof(uint8_t *));
	*pppSeqHeader = ppSeqHeader;

	uint32_t *pSeqHeaderSize = (uint32_t *)malloc((pSeqCount->GetValue() + 1) * sizeof(uint32_t *));
	...
	*ppSeqHeaderSize = pSeqHeaderSize;
	...
	bool bNumofPps = avcCAtom->FindProperty("avcC.numOfPictureParameterSets",(MP4Property **)&pPictCount);
	bool bPpsLen = avcCAtom->FindProperty("avcC.pictureEntries.pictureParameterSetLength", (MP4Property **)&pPictLen);
	bool bPpsNalU = avcCAtom->FindProperty("avcC.pictureEntries.pictureParameterSetNALUnit",(MP4Property **)&pPictVal);
	if ( !bNumofPps  || !bPpsLen|| !bPpsNalU  ) 
	{
		VERBOSE_ERROR(m_verbosity,  WARNING("Could not find avcC picture table properties"));
		return false;
	}
	...
	uint8_t 
	**ppPictHeader = (uint8_t **)malloc((pPictCount->GetValue() + 1) * sizeof(uint8_t *));
	uint32_t *pPictHeaderSize = (uint32_t *)malloc((pPictCount->GetValue() + 1)* sizeof(uint32_t *));
...
void MP4File::SetHintTrackSdp(MP4TrackId hintTrackId, const char* sdpString)
{
	...
	if (strcmp(pTrack->GetType(), MP4_HINT_TRACK_TYPE)) 
		throw new MP4Error("track is not a hint track", "MP4SetHintTrackSdp");

	AddDescendantAtoms( MakeTrackName(hintTrackId, NULL), "udta.hnti.sdp ");

	SetTrackStringProperty(hintTrackId, "udta.hnti.sdp .sdpText", sdpString);
}

void MP4File::AppendHintTrackSdp(MP4TrackId hintTrackId, const char* sdpFragment)
{
	const char* oldSdpString = NULL;
	try 
	{
...
void MP4File::GetHintTrackRtpPayload( MP4TrackId hintTrackId,
	char** ppPayloadName,
...
)
{
	...
	if (strcmp(pTrack->GetType(), MP4_HINT_TRACK_TYPE)) 
	{
		throw new MP4Error("track is not a hint track", "MP4GetHintTrackRtpPayload");
	}

	((MP4RtpHintTrack*)pTrack)->GetPayload( ppPayloadName, pPayloadNumber, pMaxPayloadSize, ppEncodingParams);
}
					 ...
					 )
{
	...
	if (strcmp(pTrack->GetType(), MP4_HINT_TRACK_TYPE)) 
	{
		throw new MP4Error("track is not a hint track", "MP4SetHintTrackRtpPayload");
	}
...
u_int8_t MP4File::AllocRtpPayloadNumber()
{
	...
	for (i = 0; i < m_pTracks.Size(); i++) 
	{
		...
		pTrakAtom->FindProperty("trak.udta.hinf.payt.payloadNumber",(MP4Property**)&pPayloadProperty);
	...
	{
		throw 
			new MP4Error("no more available rtp payload numbers",	"AllocRtpPayloadNumber");
	}

	return payload;
}

MP4TrackId MP4File::GetHintTrackReferenceTrackId(MP4TrackId hintTrackId)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
...
void MP4File::ReadRtpHint(	MP4TrackId hintTrackId,	MP4SampleId hintSampleId,	u_int16_t* pNumPackets)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
	...
	((MP4RtpHintTrack*)pTrack)->ReadHint(hintSampleId, pNumPackets);
}

u_int16_t MP4File::GetRtpHintNumberOfPackets( MP4TrackId hintTrackId)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
...
int8_t MP4File::GetRtpPacketBFrame( MP4TrackId hintTrackId, u_int16_t packetIndex)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
...
int32_t MP4File::GetRtpPacketTransmitOffset( MP4TrackId hintTrackId, u_int16_t packetIndex)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
...
void MP4File::ReadRtpPacket(MP4TrackId hintTrackId,
	...
	)
{
	...
	((MP4RtpHintTrack*)pTrack)->ReadPacket( packetIndex, ppBytes, pNumBytes,
		ssrc, includeHeader, includePayload);
}

MP4Timestamp MP4File::GetRtpTimestampStart(	MP4TrackId hintTrackId)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
...
void MP4File::SetRtpTimestampStart(	MP4TrackId hintTrackId,	MP4Timestamp rtpStart)
{
	MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
...
void MP4File::AddRtpHint(MP4TrackId hintTrackId, bool isBframe, u_int32_t timestampOffset)
{
...
void MP4File::AddRtpPacket(	MP4TrackId hintTrackId, bool setMbit, int32_t transmitOffset)
{
...
void MP4File::WriteRtpHint(MP4TrackId hintTrackId,	MP4Duration duration, bool isSyncSample)
{
...
u_int64_t MP4File::ConvertFromMovieDuration(MP4Duration duration,	u_int32_t timeScale)
{
	return MP4ConvertTime((u_int64_t)duration, GetTimeScale(), timeScale);
}

u_int64_t MP4File::ConvertFromTrackTimestamp(MP4TrackId trackId, MP4Timestamp timeStamp,u_int32_t timeScale)
{
	return MP4ConvertTime(timeStamp, GetTrackTimeScale(trackId), timeScale);
}

MP4Timestamp MP4File::ConvertToTrackTimestamp( MP4TrackId trackId, u_int64_t timeStamp, u_int32_t timeScale)
{
	return (MP4Timestamp)MP4ConvertTime(timeStamp, timeScale, GetTrackTimeScale(trackId));
}

u_int64_t MP4File::ConvertFromTrackDuration(MP4TrackId trackId, MP4Duration duration,u_int32_t timeScale)
{
	return MP4ConvertTime((u_int64_t)duration, GetTrackTimeScale(trackId), timeScale);
}

MP4Duration MP4File::ConvertToTrackDuration(MP4TrackId trackId, u_int64_t duration,u_int32_t timeScale)
{
	return (MP4Duration)MP4ConvertTime(duration, timeScale, GetTrackTimeScale(trackId));
}
...
char* MP4File::MakeTrackEditName(MP4TrackId trackId,MP4EditId editId,const char* name)
{
	...
	snprintf(m_editName, 1024,	"%s.edts.elst.entries[%u].%s", 
		trakName, editId - 1, name);

	return m_editName;
}

MP4EditId MP4File::AddTrackEdit(MP4TrackId trackId,	MP4EditId editId)
{
	ProtectWriteOperation("AddTrackEdit");
	return m_pTracks[FindTrackIndex(trackId)]->AddEdit(editId);
}

void MP4File::DeleteTrackEdit(	MP4TrackId trackId,	MP4EditId editId)
{
	ProtectWriteOperation("DeleteTrackEdit");
	m_pTracks[FindTrackIndex(trackId)]->DeleteEdit(editId);
}

u_int32_t MP4File::GetTrackNumberOfEdits(	MP4TrackId trackId)
{
	return GetTrackIntegerProperty(trackId, "edts.elst.entryCount");
}

MP4Duration MP4File::GetTrackEditTotalDuration(	MP4TrackId trackId,	MP4EditId editId)
{
	return m_pTracks[FindTrackIndex(trackId)]->GetEditTotalDuration(editId);
}

MP4Timestamp MP4File::GetTrackEditStart(MP4TrackId trackId,	MP4EditId editId)
{
	return m_pTracks[FindTrackIndex(trackId)]->GetEditStart(editId);
}

MP4Timestamp MP4File::GetTrackEditMediaStart(MP4TrackId trackId,	MP4EditId editId)
{
	return GetIntegerProperty(MakeTrackEditName(trackId, editId, "mediaTime"));
}

void MP4File::SetTrackEditMediaStart(MP4TrackId trackId,MP4EditId editId,MP4Timestamp startTime)
{
	SetIntegerProperty(	MakeTrackEditName(trackId, editId, "mediaTime"),
		startTime);
}

MP4Duration MP4File::GetTrackEditDuration(	MP4TrackId trackId,	MP4EditId editId)
{
	return GetIntegerProperty(	MakeTrackEditName(trackId, editId, "segmentDuration"));
}

void MP4File::SetTrackEditDuration(MP4TrackId trackId,	MP4EditId editId,	MP4Duration duration)
{
	SetIntegerProperty(	MakeTrackEditName(trackId, editId, "segmentDuration"),duration);
}

bool MP4File::GetTrackEditDwell(MP4TrackId trackId,	MP4EditId editId)
{
	return (GetIntegerProperty(	MakeTrackEditName(trackId, editId, "mediaRate")) == 0);
}

void MP4File::SetTrackEditDwell(MP4TrackId trackId,	MP4EditId editId,bool dwell)
{
	SetIntegerProperty(	MakeTrackEditName(trackId, editId, "mediaRate"),(dwell ? 0 : 1));
}

MP4SampleId MP4File::GetSampleIdFromEditTime(MP4TrackId trackId,
	MP4Timestamp when, MP4Timestamp* pStartTime,	MP4Duration* pDuration)
{
	return m_pTracks[FindTrackIndex(trackId)]->GetSampleIdFromEditTime(when, pStartTime, pDuration);
}
