...
public: /* equivalent to MP4 library API */
	...
	void Create(const char* fileName, u_int32_t flags, int add_ftyp = 1, 
		int add_iods = 1,
	...
	void Optimize(const char* orgFileName, const char* newFileName = NULL);
	void Dump(FILE* pDumpFile = NULL, bool dumpImplicits = false);
	void Close();

	/* library property per file */

	u_int32_t GetVerbosity() {return m_verbosity;}
	void SetVerbosity(u_int32_t verbosity) {m_verbosity = verbosity;}
	...
	void GetBytesProperty(const char* name,	u_int8_t** ppValue, u_int32_t* pValueSize);
	...
	void SetBytesProperty(const char* name, const u_int8_t* pValue, u_int32_t valueSize);
	...
	u_int64_t GetTrackIntegerProperty(MP4TrackId trackId, const char* name);
	float GetTrackFloatProperty(MP4TrackId trackId, const char* name);
	const char* GetTrackStringProperty(	MP4TrackId trackId, const char* name);
	void GetTrackBytesProperty(	MP4TrackId trackId, const char* name,u_int8_t** ppValue, u_int32_t* pValueSize);

	void SetTrackIntegerProperty(MP4TrackId trackId, const char* name, int64_t value);
	void SetTrackFloatProperty(	MP4TrackId trackId, const char* name, float value);
	void SetTrackStringProperty(MP4TrackId trackId, const char* name, const char* value);
	void SetTrackBytesProperty(	MP4TrackId trackId, const char* name, const u_int8_t* pValue, u_int32_t valueSize);
	...
	MP4SampleId GetSampleIdFromTime(MP4TrackId trackId, MP4Timestamp when, bool wantSyncSample = false);

	MP4Timestamp GetSampleTime(	MP4TrackId trackId, MP4SampleId sampleId);

	MP4Duration GetSampleDuration(	MP4TrackId trackId, MP4SampleId sampleId);

	MP4Duration GetSampleRenderingOffset(MP4TrackId trackId, MP4SampleId sampleId);

	bool GetSampleSync(	MP4TrackId trackId, MP4SampleId sampleId);

	void ReadSample(  MP4TrackId trackId, MP4SampleId sampleId,		// input parameters
		u_int8_t** ppBytes,										// output parameters
		u_int32_t* pNumBytes, 
	...
	void ReadSampleEx(
			MP4TrackId trackId, MP4SampleId sampleId, u_int32_t  SampleCount,			// input parameters
			u_int8_t** ppBytes, u_int32_t* pNumBytes,u_int32_t* pRealSampleCount = NULL // output parameters
		);
	
	void SetSampleTimeStamp(MP4Timestamp nTimeStamp);
	void WriteSample( MP4TrackId trackId, const u_int8_t* pBytes, 
		u_int32_t numBytes,
	...
	MP4TrackId AddAudioTrack(u_int32_t timeScale, MP4Duration sampleDuration,	u_int8_t audioType);
	...
	void SetAmrVendor(MP4TrackId trackId,u_int32_t vendor);
	
	void SetAmrDecoderVersion(MP4TrackId trackId,u_int8_t decoderVersion);
	
	void SetAmrModeSet(	MP4TrackId trackId,	u_int16_t modeSet);
	...
	MP4TrackId AddHrefTrack(uint32_t timeScale,	MP4Duration sampleDuration);
	...
	void SetH263Vendor(	MP4TrackId trackId,	u_int32_t vendor);
	
	void SetH263DecoderVersion(	MP4TrackId trackId,	u_int8_t decoderVersion);
	
	void SetH263Bitrates(MP4TrackId,u_int32_t avgBitrate,	u_int32_t maxBitrate);
	...
	// sps
	bool AddH264SequenceParameterSet(MP4TrackId trackId, const uint8_t *pSequence, uint16_t sequenceLen);
	
	// pps
	bool AddH264PictureParameterSet(MP4TrackId trackId,	const uint8_t *pPicture, uint16_t pictureLen);
	...
	void GetTrackESConfiguration(MP4TrackId trackId, u_int8_t** ppConfig, u_int32_t* pConfigSize);
	
	void SetTrackESConfiguration(MP4TrackId trackId, const u_int8_t* pConfig, u_int32_t configSize);

	void GetTrackVideoMetadata(MP4TrackId trackId, 	u_int8_t** ppConfig, u_int32_t* pConfigSize);

	void GetTrackH264ProfileLevel(MP4TrackId trackId,uint8_t *pProfile, uint8_t *pLevel);
	
	bool GetTrackH264SeqPictHeaders(MP4TrackId trackId, uint8_t ***pSeqHeader,
					uint32_t **pSeqHeaderSize,
					uint8_t ***pPictHeader,
					uint32_t **pPictHeaderSize);
	
	void GetTrackH264LengthSize(MP4TrackId, uint32_t *pLength);
	...
	u_int64_t ConvertFromMovieDuration(MP4Duration duration, u_int32_t timeScale);

	u_int64_t ConvertFromTrackTimestamp(MP4TrackId trackId, MP4Timestamp timeStamp,u_int32_t timeScale);

	MP4Timestamp ConvertToTrackTimestamp(MP4TrackId trackId, u_int64_t timeStamp,u_int32_t timeScale);

	u_int64_t ConvertFromTrackDuration(MP4TrackId trackId, MP4Duration duration, u_int32_t timeScale);

	MP4Duration ConvertToTrackDuration(MP4TrackId trackId, u_int64_t duration,u_int32_t timeScale);
	...
	MP4TrackId GetHintTrackReferenceTrackId(MP4TrackId hintTrackId);
	...
	u_int16_t GetRtpHintNumberOfPackets(MP4TrackId hintTrackId);

	int8_t GetRtpPacketBFrame(MP4TrackId hintTrackId,u_int16_t packetIndex);

	int32_t GetRtpPacketTransmitOffset(	MP4TrackId hintTrackId,	u_int16_t packetIndex);
	...
	MP4Timestamp GetRtpTimestampStart(MP4TrackId hintTrackId);

	void SetRtpTimestampStart(MP4TrackId hintTrackId,MP4Timestamp rtpStart);

	void AddRtpHint(MP4TrackId hintTrackId,bool isBframe, u_int32_t timestampOffset);
	...
	void AddRtpESConfigurationPacket(MP4TrackId hintTrackId);
	...
	void DeleteTrackEdit(MP4TrackId trackId,MP4EditId editId);

	u_int32_t GetTrackNumberOfEdits(MP4TrackId trackId);
 ...
 public:
	...
	/* get metadata */
	bool GetMetadataByIndex(u_int32_t index,const char** ppName,u_int8_t** ppValue, u_int32_t* pValueSize);
...
protected:
	...
	void FindIntegerProperty(const char* name, MP4Property** ppProperty, u_int32_t* pIndex = NULL);
	void FindFloatProperty(const char* name, MP4Property** ppProperty, u_int32_t* pIndex = NULL);
	void FindStringProperty(const char* name,MP4Property** ppProperty, u_int32_t* pIndex = NULL);
	void FindBytesProperty(const char* name, MP4Property** ppProperty, u_int32_t* pIndex = NULL);

	bool FindProperty(const char* name,	MP4Property** ppProperty, u_int32_t* pIndex = NULL);

	MP4TrackId AddVideoTrackDefault(u_int32_t timeScale, 
		MP4Duration sampleDuration,
		u_int16_t width, 
		u_int16_t height, 
	...
	MP4TrackId AddCntlTrackDefault(	u_int32_t timeScale, MP4Duration sampleDuration,const char *videoType);
	...
	void CreateIsmaIodFromFile(	MP4TrackId odTrackId,
	...
	MP4Descriptor* CreateESD(MP4DescriptorProperty* pEsProperty,
		u_int32_t esid,
		u_int8_t objectType,
		u_int8_t streamType,
	...
	void CreateIsmaODUpdateCommandFromFileForFile(MP4TrackId odTrackId,
		MP4TrackId audioTrackId, 
		MP4TrackId videoTrackId,
		u_int8_t** ppBytes,
		u_int64_t* pNumBytes
...
protected:
	...
	MP4Timestamp    m_nSampleTimeStamp;
