	...
	MP4TrackId trackId)
{
	...
	else if (strcasecmp(media_data_name, "mp4a") == 0) 
	{
	  ...
	  case MP4_MPEG4_AUDIO_TYPE:  
		...
		if (pAacConfig != NULL && aacConfigLength >= 2) 
		{
			...
			if (type == 0 ||  type == 10 || type == 11 || type == 18 || type >= 28) /* type == 5 || */
			{
	...
	double msDuration =  UINT64_TO_DOUBLE(MP4ConvertFromTrackDuration(mp4File, trackId,
...
static char* PrintVideoInfo( MP4FileHandle mp4File, MP4TrackId trackId)
{
	...
	u_int8_t numMpegVideoTypes = sizeof(mpegVideoTypes) / sizeof(u_int8_t);
...
extern "C" char* MP4Info( MP4FileHandle mp4File, MP4TrackId trackId )
{
	char* info = NULL;
