	...
	bool addPayload)
{
	...
	*pNumBytes = 0;
	...
	try {
		...
		if (addHeader) 
		{
			*pDest++ =	0x80 | (pPacket->GetPBit() << 5) | (pPacket->GetXBit() << 4);

			*pDest++ =	(pPacket->GetMBit() << 7) | pPacket->GetPayload();

			*((u_int16_t*)pDest) = 	htons(m_rtpSequenceStart + pPacket->GetSequenceNumber());
			pDest += 2; 

			*((u_int32_t*)pDest) = 	htonl(m_rtpTimestampStart + (u_int32_t)m_readHintTimestamp);
			pDest += 4; 

			*((u_int32_t*)pDest) = 	htonl(ssrc);
			pDest += 4;
		}
...
void MP4RtpHintTrack::GetPayload(char** ppPayloadName,
...
void MP4RtpHintTrack::SetPayload(const char* payloadName,
