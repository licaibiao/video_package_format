...
MP4Atom* MP4Atom::CreateAtom(const char* type)
{
	...
	if (NULL == pAtom ) 
		pAtom = new MP4StandardAtom(type);// unknown type is set by StandardAtom type
...
void MP4Atom::Generate()
{
	...
	for (i = 0; i < m_pProperties.Size(); i++) 
	{
		m_pProperties[i]->Generate();		// ask it to self generate
	}

	// for all mandatory, single child atom types
	for (i = 0; i < m_pChildAtomInfos.Size(); i++) 
	{
		if (m_pChildAtomInfos[i]->m_mandatory  && m_pChildAtomInfos[i]->m_onlyOne)
		{
...
MP4Atom* MP4Atom::ReadAtom(MP4File* pFile, MP4Atom* pParentAtom)
{
	...
	if (dataSize == 0)
		dataSize = pFile->GetSize() - pos; // extends to EOF
	...
	if (pAtom->IsUnknownType()) 
	{
		...
		if (dataSize > 0)
		{
			pAtom->AddProperty(	new MP4BytesProperty("data", dataSize));
		}
	}
...
MP4Atom* MP4Atom::FindAtom(const char* name)
{
	...
	if (!IsRootAtom()) 
	{
		VERBOSE_FIND(m_pFile->GetVerbosity(), printf("FindAtom: matched %s\n", name));
...
bool MP4Atom::FindProperty(const char *name, MP4Property** ppProperty, u_int32_t* pIndex)
{
	if (!IsMe(name))  
		return false;
	...
	if (!IsRootAtom()) 
	{
		VERBOSE_FIND(m_pFile->GetVerbosity(), printf("FindProperty: matched %s\n", name));
		name = MP4NameAfterFirst(name);
...
void MP4Atom::ReadChildAtoms()
{
	...
	VERBOSE_READ(GetVerbosity(), printf("ReadChildAtoms: finished %s\n", m_type));
}
...
void MP4Atom::Rewrite()
{
	...
	if (!m_end) 
	   return;// This atom hasn't been written yet...
...
void MP4Atom::WriteProperties(u_int32_t startIndex, u_int32_t count)
{
	...
	VERBOSE_WRITE(GetVerbosity(), printf("Write: type %s\n", m_type));
	...
	for (u_int32_t i = startIndex; i < startIndex + numProperties; i++) 
	{
		...
		{
			VERBOSE_WRITE(GetVerbosity(), printf("Write: "); m_pProperties[i]->Dump(stdout, 0, false));
		}
	}
}
...
void MP4Atom::WriteChildAtoms()
{
	...
	VERBOSE_WRITE(GetVerbosity(), printf("Write: finished %s\n", m_type));
}
...
void MP4Atom::Dump(FILE* pFile, u_int8_t indent, bool dumpImplicits)
{
	...
	for (i = 0; i < size; i++) 
	{
		...
		if ( (m_pProperties[i]->GetType()==TableProperty)&& !(GetVerbosity()&MP4_DETAILS_TABLE ) )
		{
