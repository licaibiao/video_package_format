...
MP4Track::MP4Track(MP4File* pFile, MP4Atom* pTrakAtom) 
{
	...
	// Added by ding yong 2008-11-17
	
	m_mp4Index = NULL;
//	DWORD frameNum = GetNumberOfSamples();
//	if (m_bFrameNum != 0x090909)
//	{
//		m_nFrameNum = frameNum;
//		m_bFrameNum = 0x090909;
//	}
//	m_mp4Index = new MP4Index[m_nFrameNum];
	if (strcmp(GetType(), MP4_VIDEO_TRACK_TYPE) == 0)
	{
	
		DWORD frameNum = GetNumberOfSamples();
		m_mp4Index = new MP4Index[frameNum];
		if (m_mp4Index == NULL)
		{
			throw new MP4Error("Not enough memory", "MP4Track::MP4Track");
		}
		for (int idx=1; idx<=frameNum; idx++)
		{
			
			m_mp4Index[idx-1].pFile = GetSampleFile(idx);
			m_mp4Index[idx-1].dwOffset = GetSampleFileOffset(idx);
			m_mp4Index[idx-1].dwSize = GetSampleSize(idx);
		}
	}
	//end
...
MP4Track::~MP4Track()
{
	...
	if (m_mp4Index != NULL)
	{
		//printf("====== ¡ï¡ï¡ï¡ïdelete m_mp4Index¡ï¡ï¡ï¡ï ======\r\n");
		delete [] m_mp4Index;
		m_mp4Index = NULL;
	}
}
	...
	bool* pIsSyncSample)
{
	...
	if (strcmp(GetType(), MP4_VIDEO_TRACK_TYPE) != 0)
	{
		...
		fileOffset = GetSampleFileOffset(sampleId);
	...
	{
		pFile = m_mp4Index[sampleId-1].pFile;
		fileOffset = m_mp4Index[sampleId-1].dwOffset;
		sampleSize = m_mp4Index[sampleId-1].dwSize;	
	}
	...
	u_int32_t* pRealSampleCount)
{
	...
	int nLeftSample = SampleCount;
	int nChunkLeftSample = 1;
	int nReadSample = 0;
	BYTE* pDataPtr = NULL;
	int nDataBufSize = 0;

	u_int32_t sampleTotalSize = 0;
	int beginSampleID = sampleId;
	for(int i= 0; i<SampleCount; i++)
	{
		u_int32_t sampleSize = GetSampleSize(beginSampleID);
		if(sampleSize > 0)
		{
	...
	bool bufferMalloc = false;
	if (*ppBytes == NULL || (*pNumBytes < sampleTotalSize)) 
	{
		if(*ppBytes)
		{
			MP4Free(*ppBytes);
		...
		*ppBytes = (u_int8_t*)MP4Malloc(sampleTotalSize);
		bufferMalloc = true;
	}
	*pNumBytes = sampleTotalSize;
	pDataPtr = (BYTE*)*ppBytes;

	while(nLeftSample && (nChunkLeftSample > 0))
	{
		u_int64_t fileOffset = GetSampleFileOffsetEx(sampleId, nChunkLeftSample);
//Add by cwl
#ifndef WIN32
#define min(a,b)            (((a) < (b)) ? (a) : (b))
#endif
		nReadSample = min(nChunkLeftSample, nLeftSample);
		...
		{ 
			//only for LPCM 
			u_int32_t sampleSize = GetSampleSize(beginSampleID);
			u_int32_t nReadSize = nReadSample * sampleSize;
			m_pFile->SetPosition(fileOffset, pFile);
			m_pFile->ReadBytes((u_int8_t*)pDataPtr, nReadSize, pFile);
			pDataPtr += nReadSize;
		...
		sampleId += nReadSample;
		nLeftSample -= nReadSample;
		if(pRealSampleCount)
			*pRealSampleCount += nReadSample;
	}
	
}


void MP4Track::ReadSampleFragment(MP4SampleId sampleId,
	...
	u_int8_t* pDest )
{
	...
	if (sampleId != m_cachedReadSampleId) 
	{
		...
		ReadSample(	sampleId,&m_pCachedReadSample,&m_cachedReadSampleSize);
...
void MP4Track::WriteSample(	const u_int8_t* pBytes, 
	u_int32_t numBytes,
	...
	)
{
	...
	if ((m_isAmr == AMR_TRUE) &&(m_curMode != curMode)) 
	{
		WriteChunkBuffer();
		m_curMode = curMode;
	}
	...
	m_pChunkBuffer = (u_int8_t*)MP4Realloc(m_pChunkBuffer,	m_chunkBufferSize + numBytes);
	...
	UpdateModificationTimes( nTimestamp );
...
void MP4Track::FinishWrite()
{
	...
	if (m_pTrakAtom->FindProperty( "trak.mdia.minf.stbl.stsd.*.esds.decConfigDescr.bufferSizeDB",
	...
	if (m_pTrakAtom->FindProperty( "trak.mdia.minf.stbl.stsd.*.esds.decConfigDescr.avgBitrate",
	  (MP4Property**)&pBitrateProperty)) {
...
u_int64_t MP4Track::GetSampleFileOffsetEx(MP4SampleId sampleId, int& nChunkLfSampCount)
{
	u_int32_t stscIndex =
		GetSampleStscIndex(sampleId);
	...
	nChunkLfSampCount = samplesPerChunk - (sampleId - firstSampleInChunk);
...
void MP4Track::UpdateModificationTimes( MP4Timestamp nTimestamp )
{
	// update media and track modification times
	MP4Timestamp now = MP4GetAbsTimestamp();
	m_pMediaModificationProperty->SetValue( nTimestamp );
	m_pTrackModificationProperty->SetValue( nTimestamp ); //now
}
